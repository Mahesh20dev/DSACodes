// Longest Common Subsequence(DP) part-3
public class Solution {
    ...
    // Memoization (Top down)  Time and space O(n*m)
    ...
    // Tabulation (bottom up)   Time and space = O(n * m)
    public static int lcs3(String str1, String str2) {
        ...
        for(int i=0; i<dp.length; i++) {
            for(int j=0; j<dp[0].length; j++) {
                if(i == 0 || j == 0) {
                    dp[i][j] = 0;             // No need because default it stores 0
                } 
            }
        }
